...
// OpenMP-based polynomial multiplication
std::vector<ll> multiply_ntt_crt_linear_openmp(
    ...
    if (selected_primes.empty()) {
        ...
        throw std::runtime_error("OpenMP: No CRT primes selected.");
    }
    ...
    if (n_fft > 0) {
        #pragma omp parallel for schedule(dynamic)
        for (int i = 0; i < static_cast<int>(n_fft); ++i) { 
            if (selected_primes.empty()) {
                final_coeffs[i] = 0;
            } else {
...
std::vector<ll> multiply_naive_openmp(const std::vector<ll>& poly1, const std::vector<ll>& poly2, ll mod) {
...
void print_poly_openmp(const std::string& name, const std::vector<ll>& p, std::ostream& os = std::cout) {
    os << name << " (size " << p.size() << "): [";
...
bool compare_polys_openmp(const std::vector<ll>& p1, const std::vector<ll>& p2) {
    if (p1.size() != p2.size()) {
...
int main(int argc, char *argv[]) {
    ...
    try {
        std::vector<ll> result = multiply_ntt_crt_linear_openmp(poly_A, poly_B, n_coeffs, p_target, num_threads);
        for (size_t i = 0; i < result.size(); ++i) {
            std::cout << result[i] << (i == result.size() - 1 ? "" : " ");
        }
        std::cout << std::endl;
    } catch (const std::exception& e) {
        std::cerr << "Exception in openmp execution: " << e.what() << std::endl;
        return 1;
    }
