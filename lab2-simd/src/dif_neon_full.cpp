...
inline uint32x4_t reduce4(uint64x2_t w0, uint64x2_t w1) {
    uint32x4_t t0 = { static_cast<uint32_t>(vgetq_lane_u64(w0, 0) >> M::w),
                      static_cast<uint32_t>(vgetq_lane_u64(w0, 1) >> M::w),
                      static_cast<uint32_t>(vgetq_lane_u64(w1, 0) >> M::w),
                      static_cast<uint32_t>(vgetq_lane_u64(w1, 1) >> M::w) };
    uint32x4_t w32 = { static_cast<uint32_t>(vgetq_lane_u64(w0, 0)),
                       static_cast<uint32_t>(vgetq_lane_u64(w0, 1)),
                       static_cast<uint32_t>(vgetq_lane_u64(w1, 0)),
                       static_cast<uint32_t>(vgetq_lane_u64(w1, 1)) };
    uint32x4_t t1 = vmulq_n_u32(w32, M::i);
    uint64x2_t t2_0 = vmull_n_u32(vget_low_u32(t1), M::m);
    uint64x2_t t2_1 = vmull_n_u32(vget_high_u32(t1), M::m);
    uint32x4_t t3 = { static_cast<uint32_t>(vgetq_lane_u64(t2_0, 0) >> M::w),
                      static_cast<uint32_t>(vgetq_lane_u64(t2_0, 1) >> M::w),
                      static_cast<uint32_t>(vgetq_lane_u64(t2_1, 0) >> M::w),
                      static_cast<uint32_t>(vgetq_lane_u64(t2_1, 1) >> M::w) };
    uint32x4_t m_vec = vdupq_n_u32(M::m);
    uint32x4_t res = vaddq_u32(t0, vsubq_u32(m_vec, t3));
    return res;
}
