...
// Updated to use the three primes identified as suitable for N=131072 and P_target=1.337e15
const ll M0_CRT = 6597069766657LL;    // Former P_A
const ll G0_CRT = 5;
const ll M1_CRT = 2748779069441LL;    // Former P_B
const ll G1_CRT = 3;
const ll M2_CRT = 2061584302081LL;    // Former P_C
const ll G2_CRT = 7;
...
std::vector<ll> multiply_ntt_crt_for_linear(const std::vector<ll>& poly_a_orig, const std::vector<ll>& poly_b_orig, int n_orig_coeffs, ll final_p_target) {
    if (n_orig_coeffs == 0) return {};

    int linear_conv_len = 2 * n_orig_coeffs - 1;
    if (n_orig_coeffs == 1) linear_conv_len = 1; // Special case: const * const
    
    int n_fft = 1;
    while(n_fft < linear_conv_len) n_fft <<= 1;
    if (linear_conv_len == 0) n_fft = 0; // Should not happen if n_orig_coeffs > 0
    ...
    for (int k = 0; k < 3; ++k) {
        ...
        // Coefficients are already taken from original poly_a/b which are u64.
        // Here, they are converted to ll and then taken modulo CRT_MODS[k].
        for(int i = 0; i < n_fft; ++i) { // Ensure all elements are modded, esp. padded zeros.
            temp_a[i] %= CRT_MODS[k];
            if (temp_a[i] < 0) temp_a[i] += CRT_MODS[k]; // Ensure positive modulo
            temp_b[i] %= CRT_MODS[k];
            if (temp_b[i] < 0) temp_b[i] += CRT_MODS[k];
        }

        ntt(temp_a, false, CRT_MODS[k], CRT_ROOTS[k]);
        ntt(temp_b, false, CRT_MODS[k], CRT_ROOTS[k]);

        for (int i = 0; i < n_fft; ++i) {
            ntt_results_mod_crt_M[k][i] = (int128)temp_a[i] * temp_b[i] % CRT_MODS[k];
        }
        ntt(ntt_results_mod_crt_M[k], true, CRT_MODS[k], CRT_ROOTS[k]);
    }
...
int main(int argc, char *argv[])
{
    ...
    int test_begin = 0;
    int test_end = 4;
    // Test case 0: n=4. Others: n=131072
    // Test case 4 uses P_TARGET = 1337006139375617 (the large one)
    ...
    for(int i = test_begin; i <= test_end; ++i){
        ...
        if (n_coeffs_from_file > 0) {
            ...
            try {
                result_vec = multiply_ntt_crt_for_linear(poly_a_vec, poly_b_vec, n_coeffs_from_file, p_target_from_file);
