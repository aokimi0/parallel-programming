...
void ntt_transform_openmp(std::vector<long long>& a, bool invert, long long mod, long long primitive_root, int num_threads) {
    int n = a.size();
    if (n == 0) return;
...
std::vector<long long> multiply_ntt_openmp(
    ...
    ntt_transform_openmp(poly1, false, mod, primitive_root, num_threads);
    ntt_transform_openmp(poly2, false, mod, primitive_root, num_threads);
    ...
    ntt_transform_openmp(result, true, mod, primitive_root, num_threads);
...
void fWrite(u64 *ab, int n, int input_id){
    ...
    std::string str2 = std::to_string(input_id);
    std::string strout = str1 + str2 + "_openmp_arm.out";
...
void ntt_multiply_wrapper(u64 *a, u64 *b, u64 *ab, int n, u64 p, int num_threads = 4){
    ...
    std::vector<long long> result = multiply_ntt_openmp(poly1, poly2, p, primitive_root, num_threads);
...
{
    std::cout << "=== NTT OpenMP ARM 版本测试 ===" << std::endl;
